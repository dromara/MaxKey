...
<script type="text/javascript">
		...
		function onSelectRow(id){
				$.loading();
				var roleId=$.gridSelIds("#list");
				$("#roleId").val(roleId);
				$.fn.zTree.getZTreeObj("tree").checkAllNodes(false);
				$.post('<c:url value="/roleNav/queryNavs"/>/'+roleId, {currTime:(new Date()).getTime()}, function(data) {
					if(data&&data.length){
	...
	$(function () {
			...
			$("#roleNavsSaveBtn").click(function(){
	 		...
	 		<table   class="datatable">
 				<tr>
				 	...
				 	<td colspan="2"> <div id="tool_box_right">
							<input  class="button" id="roleNavsSaveBtn" type="button" value="<s:Locale code="button.text.save"/>">
				 	</div>
				 	</td>
				</tr>
			...
			<tbody>
				<tr>
					...
					<td valign="top"  class="td_1" width="70%" style="vertical-align: top;">
						...
						<s:Tree rootId="<%=WebContext.getSystemNavRootId()%>" url="/navigations/tree" id="tree" checkbox="true"/>
					</td>
				</tr>
			</tbody>
		</table>
...
<div style="display:none">
	<form id="actionForm" method="post" action="<c:url value='/roleNav/insert'/>">
		<table>
			<tr><td></td><td><input type="text" id="roleId" name="id" value=""/></td></tr>
			<tr><td></td><td><input type="text" id="navsId" name="navsId" value=""/></td></tr>
