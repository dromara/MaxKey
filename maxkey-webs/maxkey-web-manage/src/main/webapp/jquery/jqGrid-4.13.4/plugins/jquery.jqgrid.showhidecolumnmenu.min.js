...
c=0;c<k;c++)u[g[c].name]=c;if(null!=p&&null!=p.groupHeaders)for(r=0,v=p.groupHeaders.length;r<v;r++)for(n=p.groupHeaders[r],t=0;t<n.numberOfColumns;t++)c=u[n.startColumnName]+t,f=g[c],q[c]=a.isFunction(b.buildItemText)?b.buildItemText.call(d[0],{iCol:c,cm:f,cmName:f.name,colName:h[c],groupTitleText:n.titleText}):a.jgrid.stripHtml(n.titleText)+": "+a.jgrid.stripHtml(""===h[c]?f.name:h[c]);for(c=0;c<k;c++)void 0===q[c]&&(f=g[c],q[c]=a.isFunction(b.buildItemText)?b.buildItemText.call(d[0],{iCol:c,cm:f,
cmName:f.name,colName:h[c],groupTitleText:null}):a.jgrid.stripHtml(h[c]));for(c=0;c<k;c++)f=g[c],h=a("<li></li>").data("iCol",c).html(q[c]),b.modifyMenuItem.call(d[0],h,f,b),h.prepend(f.hidden?b.checkboxUnChecked:b.checkboxChecked),e&&h.wrapInner("<a></a>"),h.appendTo(l);l.css(b.menuStyle);a("ul.ui-jqgrid-showHideColumnMenu").menu("destroy").remove();l.appendTo("body").menu({select:function(c,e){var m=parseInt(e.item.data("iCol"),10),h=e.item.find(b.checkboxSelector),f=g[m],k=b.isChecked.call(d[0],
h,c,f);!isNaN(m)&&0<=m&&null!=f&&0<h.length&&(k?(b.toUnCheck.call(d[0],h,c,f),d.jqGrid("hideCol",f.name)):(b.toCheck.call(d[0],h,c,f),d.jqGrid("showCol",f.name)),a(this).parent().css("zoom",1),l.menu("focus",c,e.item))},create:function(){var a=l.height(),b=window.innerHeight||document.documentElement.clientHeight;a>b&&l.height(b).css("overflow-y","scroll")}}).mouseleave(function(){a(this).menu("destroy").remove()}).position({of:a(m.target),my:"left top",at:"right center",collision:"flipfit flipfit"});
...
checkboxUnChecked:'<input disabled="disabled" type="checkbox"/>',checkboxSelector:"input[type=checkbox]",isChecked:function(a){return a.is(":checked")},toCheck:function(a){a.prop("checked",!0)},toUnCheck:function(a){a.prop("checked",!1)}},b);k.call(this);d.bind("jqGridAfterSetGroupHeaders",function(){k.call(this)})})}})});
